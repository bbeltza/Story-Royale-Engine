...
void sregl11_destroy(void* _inst)
{
...
bool sregl11_main(const struct sre_RenderVFT** vft, void* _inst, SDL_Window* window)
{
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MAJOR_VERSION, 1);
    SDL_GL_SetAttribute(SDL_GL_CONTEXT_MINOR_VERSION, 1);

    sregl11_inst* inst = _inst;
    if (!sregl_commonsetup(&inst->common, window, &inst->glfuncs))
        return false;
