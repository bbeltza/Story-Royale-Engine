...
void AudioData::Load()
{
	File::Chunk data_chunk = File(path).allocate();

    SDL_RWops *audio_rw = SDL_RWFromConstMem(data_chunk.data, static_cast<int>(data_chunk.size));
    ...
    else if ((m_len = stb_vorbis_decode_memory((uint8_t *)data_chunk.data, (int)data_chunk.size, &channels_int, &m_spec.freq, (short **)&m_data)) > 0)
    {
