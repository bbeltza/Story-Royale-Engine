...
typedef std::unordered_map<Game::Entity *, std::vector<Game::Components::Shape *>> p_stack;

const double pi = M_PI;
static p_stack CollisionStack;
static p_stack ColliderStack;

static bool colliderProcess(Game::Entity *ent, Game::Components::Shape *shape)
{
    if (shape->flags & CAN_COLLIDE)
    {
        ColliderStack[ent].push_back(shape);
        return true;
    }
...
static bool collisionProcess(Game::Entity *ent, Game::Components::Shape *shape)
{
    if (shape->flags & CAN_COLLIDE)
    {
        CollisionStack[ent].push_back(shape);
        return true;
    }
...
void Game::Entity::_pProcess(float dt)
{
    ...
    Components::Velocity *vComp = velocityComp;
    if (!velocityComp || vComp->x_direction == 0 && vComp->y_direction == 0 || vComp->velocity == 0)
        ;
    else
    {
        vComp->move(this, dt);
        ...
        {
            if (c->getProcessFlags() & Component::p_pUpdate)
                colliderProcess(this, (Game::Components::Shape *)c);
        }
    }
    for (auto c : m_Components)
    {
        if (c->getProcessFlags() & Component::p_pUpdate)
            collisionProcess(this, (Game::Components::Shape *)c);
    }
}
...
{
    if (abs(c2y - c1y) < r2.h / 2)
        collideRectX(res);
    else if (abs(c2x - c1x) < r2.w / 2)
        collideRectY(res);
    else
    {
        collideCircle(comp,
...
{
    for (auto &kv : ColliderStack)
    {
        kv.second.clear();
    }
        
    for (auto &kv : CollisionStack)
    {
        kv.second.clear();
    ...
    for (auto &kv : ColliderStack)
    {
        ...
        {
            auto c1 = kv.second.back();
            kv.second.pop_back();
            for (auto &kv_2 : CollisionStack)
            {
                c2_ent = kv_2.first;
                if (c1_ent == c2_ent)
                    continue;
                //printf("%d\n", kv_2.second.size());
                for (auto c2 : kv_2.second)
                {
                    ...
                    SDL_FRect res;
                    if (!SDL_IntersectFRect(&r1, &r2, &res))
                        continue;
