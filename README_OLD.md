...
sre_property(Game PROPERTIES TITLE "My Game") # This function does add some additional data for your game, like the title...
...
sre_build(Game GameExecutable SOURCES) # This function does all of the heavylifting. Taking your SOURCES variable as the third argument and making an executable out of it from the second one, with the engine linked. There are some options though that you can add as arguments:
...
void myUpdateFunction()
{
    sre::log("My framerate is: %g", 1 / sre::dt); // Print the framerate into the console
                                            // Should be around 39.xx if you set the framerate to 40
}
...
void sre::initialize()
{
    ...
    sre::log("Hello World!");
    sre::onUpdate.connect(myUpdateFunction); // Connect myUpdateFunction to a `Signal` that will be run every frame
    // Signals are objects that store functions to be called when fired 
    // sre::onUpdate is a signal that fires once every frame
    // The second parameter would be an address passed by the connection, in this case, we don't need it since we're passing a function with no arguments, so we don't pass anything.
}
```
