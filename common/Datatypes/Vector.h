...
#define SRE_VEC2MAKE(type, n) using n = sre::vec2<type>
#define SRE_VEC2MAKESFFX(type, n) using sre_vec2##n = sre::vec2<type>

#else

// C compatible vector 2 class template
#define SRE_VEC2MAKE(type, n) typedef struct n { type x; type y; } n
#define SRE_VEC2MAKESFFX(type, n) SRE_VEC2MAKE(type, sre_vec2##n)
#define SRE_VEC2SCALAR(x) { x, x }

#define sre_vec2add(v1, v2) do { (v1).x += (v2).x; (v1).y += (v2).y } while (0)
