...
namespace sre
{
    ...
    public:
        ...
        FontAtlas(const sre::RAIITexture& texture, sre::vec2i contant_size);
        ...
        FontAtlas(const sre::RAIITexture& texture, const sre::Glyph glyphs[CHARACTER_COUNT], sre::unit lineskip, bool normalize=true);
        
        sre::RAIITexture texture() const { return m_atlas; }
    ...
    class Font
    {
        struct FontData
        {
            FontData(FT_FaceRec_* face, const sre::RAIITexture& texture, const sre::Glyph regions[FontAtlas::CHARACTER_COUNT], sre::unit lineskip): ftface(face), atlas(texture, regions, lineskip) {}
            ~FontData();
