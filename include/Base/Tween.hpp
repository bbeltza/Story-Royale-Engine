...
namespace sre
{
    ...
    private:
        ...
        int m_state = 0;
    public:
        Tween& play() { m_state = 1; return *this; }
    ...
    private:
        ...
        template <typename T>
        class tw<T, const T*>: public Tween
        {
            T& m_target;
            const T* m_goal;
        ...
        public:
            tw(const TweenStyle& style, const T* goal, T& target): Tween(style), m_goal(goal), m_target(target) {}
        };

        template <typename T, typename GoalType>
        Tween& add_tw(const TweenStyle& style, GoalType goal, T& target)
        {
    ...
    public:
        ...
        Tween& tween(const TweenStyle& style, T& target, const T2& goal) { return add_tw<T, T>(style, goal, target); }

        template <typename T, typename T2>
        Tween& tween(const TweenStyle& style, T& target, const T2* goal) { return add_tw<T, const T*>(style, goal, target); }
    };
}
